    ...
    for (const offer of dataArr.offer) {
      await prisma[nameDB].create({
        data: {
          ...
          waterSupply: offer.waterSupply?.length && offer.waterSupply === 'да' ? 'Да' : 'Нет',
          electricitySupply:
            offer.electricitySupply?.length && offer.electricitySupply === 'да' ? 'Да' : 'Нет',
          gasSupply: offer.gasSupply?.length && offer.gasSupply === 'да' ? 'Да' : 'Нет',
          heatingSupply: offer.heatingSupply?.length && offer.heatingSupply === 'да' ? 'Да' : 'Нет',
